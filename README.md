...
| `cockpit-control` | Own the explicit versioned VP3 control store: `init`, `validate`, `publish-event`, `list-events`, `record-lifecycle`, `lifecycle-status`, `register-command`, `acknowledge-command`, `command-status`, `replay-ledger`, read-only `preflight`, and the guarded `repair-lock` / `repair-store` diagnostics. | yes |
