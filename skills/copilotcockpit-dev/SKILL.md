...
| 4 | **Integration smoke** | `bats` (`tests/integration/smoke.bats`) | `global --dry-run` enumerates all 8 skills + cockpit-wake + cockpit-overseer + cockpit-trace; `e2e <tmp> --dry-run` prints the scaffold file list; `doctor` exits 0 | CI-on-PR + post-release | must pass |
